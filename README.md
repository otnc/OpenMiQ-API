...
6. The same `DISCORD_BOT_TOKEN` is required at runtime, too — it's how the server fetches Discord avatars: the logged-in user's own avatar in the header, every avatar in the Admin user list, and the homepage's sample quote image (the first `ADMIN_DISCORD_IDS` entry's avatar). All of these go through Discord's `GET /users/{id}` (a global endpoint, unrelated to the webhook/guild setup above).
...
| `PLAYGROUND_SHARED_KEY_LIMIT` | Lets `/playground` be used without signing up first — a visitor who leaves its API key field blank falls back to a key this app generates and manages entirely on its own (never written to config anywhere). `0` (default) disables the whole feature; a positive number is how many anonymous requests that shared key may serve per rotation (`PLAYGROUND_SHARED_KEY_ROTATE_MINUTES`) before further ones are refused |
| `PLAYGROUND_SHARED_KEY_ROTATE_MINUTES` | How often the shared key above is thrown away and replaced with a freshly-generated one (default `30`) |
| `PLAYGROUND_RATE_LIMIT_WINDOW_MS` / `PLAYGROUND_RATE_LIMIT_MAX` | A separate, small per-IP-address rate limit that only applies to anonymous (no-API-key) `/playground` requests, on top of `PLAYGROUND_SHARED_KEY_LIMIT`'s total — keeps a single visitor from exhausting that shared allocation for everyone else (default `60000`/`5`) |
