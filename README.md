...
| `PLAYGROUND_API_KEY` | Optional. The plaintext of a real API key that `/playground` falls back to when a visitor leaves its own API key field blank, so anyone can try the API without signing up first. Unset = the playground still works, just only for visitors who supply their own key |
| `PLAYGROUND_RATE_LIMIT_WINDOW_MS` / `PLAYGROUND_RATE_LIMIT_MAX` | A separate, small per-IP-address rate limit that only applies to anonymous (no-API-key) `/playground` requests, on top of `PLAYGROUND_API_KEY`'s own rate limit — keeps a single visitor from exhausting that shared key's window for everyone else (default `60000`/`5`) |
