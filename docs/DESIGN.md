...
- **Web側（決定・実装済み）**: 不一致を検知した利用者には、他のConsole機能より先に「利用規約・プライバシーポリシーが更新されました」という再同意画面を表示する。**既定表示は旧バージョンとの差分（変更点サマリ）**とし、画面上のトグルで**全文表示に切り替え可能**にする。`apps/api/src/legal/content.ts`が`TERMS_VERSION`/`PRIVACY_VERSION`ごとの本文をバージョン履歴として保持し、`apps/api/src/legal/diff.ts`（`diff`パッケージの`diffWords`を利用、単語単位の差分）が直近の同意済みバージョンとの差分を生成する。`GET /api/legal/terms/diff`・`GET /api/legal/privacy/diff`（`from`クエリで比較元バージョンを指定）がこの差分を返し、`/console/reconsent`ページ（`+page.server.ts`）が`GET /api/console/me`の`agreedTermsVersion`/`agreedPrivacyVersion`を`from`として渡して取得する。差分が取得できない場合（比較元バージョンが履歴に無い等）は`available: false`が返り、ページは全文表示にフォールバックする。
