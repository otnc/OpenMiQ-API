...
- `POST /api/quote` — クォート画像生成
  - body例: `{ "authorName": "...", "authorUsername": "...", "authorAvatarUrl": "...", "text": "...", "theme": "sunset", "font": "pop", "watermark": "...", "options": { "color": true, "bold": false, "layout": "new", "hosted": false } }`（`authorName`は太字の表示名の行、`authorUsername`（任意）はその下に描画される小さな「@ユーザー名」の行 — 省略時は`authorName`と同じ文字列が両方の行に使われる（`renderService.ts`が`authorUsername ?? authorName`にフォールバック）。`watermark`はテキストの上書きだが、画像の上書きとして`watermarkUrl`（URL、`watermark`より優先）も指定可能。手元の画像ファイルを直接渡したい場合はJSONに埋め込まず、先に`POST /api/uploads`（multipart/form-data）でアップロードしてURLを取得し、それを`authorAvatarUrl`/`watermarkUrl`として渡す — base64化してこのbody自体に含める方式は、リバースプロキシの`client_max_body_size`（nginx既定1MB）に引っかかりやすく採用しなかった）
...
- 申請文章（`message`）は**バッククォート（`` ` ``）とバックスラッシュ（`\`）を禁止**（`packages/shared/src/application.ts`の`applicationMessageSchema`、正規表現バリデーション）。理由: 審査Webhookの埋め込み（§6.1の`reviewEmbed()`）は申請文章を```` ``` ````コードブロックで囲んで送信しており、本文にバッククォートが含まれるとコードブロックを閉じてしまい、埋め込み内に任意のDiscord Markdown/メンションを注入できてしまうため。Web UI側（`/console/apply`）は入力中にこの2文字をその場で除去し、サーバー側でも同じ正規表現で拒否する二重防御
...
実際にデプロイする際に上から実行できる形にまとめたチェックリストは [docs/DEPLOYMENT.md](./DEPLOYMENT.md)、そのままコピーできるnginx設定は [deploy-example/nginx/openmiq-api.conf](../deploy-example/nginx/openmiq-api.conf) を参照（実装済み、PLAN.md Phase 8）。
...
- `set*()`群（実装済み）: `setText` / `setUsername`（`authorName`にマップ — 太字の表示名の行） / `setAuthorUsername`（`authorUsername`にマップ — その下の小さな「@ユーザー名」の行、既定`null`で`authorName`と同じ文字列が両方の行に使われる） / `setAvatar`（URL文字列/`URL`、または生の画像バイト列） / `setTheme` / `setFont` / `setColor` / `setBold` / `setLayout('side'|'new')` / `setWatermark(string | AvatarSource | null)`（文字列はテキスト、URL/生バイト列は画像） / `setFake()`（`/api/fakequote`への切り替え） — §8.5 の実際の`/api/quote`スキーマ（`packages/shared/src/quote.ts`）に1:1対応させる。生の画像バイト列を渡した場合、その場ではbase64化などせず保持だけしておき、`toBuffer()`/`toURL()`実行時に`POST /api/uploads`へアップロードしてから返ってきたURLを`authorAvatarUrl`/`watermarkUrl`として送信する（往復が1回増える）。`setFromMessage`/`setFromNote`/`setFromTweet`は表示名の解決方法（ニックネーム/グローバル名優先など）に関わらず、必ずアカウント本来のハンドル（Discordの`author.username`、Misskeyノートの`user.username`/`user.host`、ツイート投稿者の`username`）を`authorUsername`に設定する。実装時の補正: `setFlip`に対応するフィールドは実際の`/api/quote`スキーマに存在しないため実装していない。`setWatermark`は`null`（既定）でサーバー側のデフォルト（`LOGO_PATH`が設定されていればそのロゴ画像、無ければmakeitaquote自体の既定）に委ね、文字列（空文字列を含む）を渡すとそれで上書きする — OpenMiQ本体（Discordボット）・OpenMiQ-misskeyの「`LOGO_PATH`をwatermarkに使う」慣習をAPI越しに再現したもの。
