...
| Discord OAuth2 | **自前実装（`ofetch`ベース、決定・変更）** | Arctic, `@badgateway/oauth2-client`, openid-client | 当初Arcticを予定していたが、実装着手時点でnpm上**deprecated（"Package no longer supported"）**と判明したため不採用に変更。Discord OAuth2は認可コードフロー2〜3回のHTTPリクエスト（認可URL組立→`POST https://discord.com/api/oauth2/token`でのコード交換→`GET https://discord.com/api/users/@me`でのユーザー取得）で完結し、既に採用済みの`ofetch`（§6.4）だけで十分に薄く書けるため、単一プロバイダのためだけに外部OAuth2クライアントライブラリを追加しない。CSRF対策の`state`は`ransu/secure`の`token()`で生成しhttpOnly Cookieに一時保存して照合する |
...
| Discord Interactions署名検証 | **`discord-interactions`**（公式提供の軽量パッケージ） | 自前Ed25519検証 | 公式実装を使い、検証ロジックの実装ミスを避ける |
