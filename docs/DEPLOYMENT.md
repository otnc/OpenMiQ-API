...
# デプロイ手順書（miq.otnc.dev、または自前のセルフホスト先）

[DESIGN.md](./DESIGN.md) §14 の設計を、実際に本番デプロイする際にそのまま上から実行できるチェックリストにしたもの。PLAN.md Phase 8に対応する。

以下は`miq.otnc.dev`（著作権者本人の運用インスタンス）を前提に書いているが、自分の別ドメインでセルフホストする場合は該当箇所をすべて読み替えること。
...
| `APP_BASE_URL` | `https://miq.otnc.dev`（自分のドメインの場合は読み替え） |
...
| `ICON_PATH` / `LOGO_PATH` | **著作権者本人のデプロイ（miq.otnc.dev）では設定不要** — 未設定時のフォールバック先である`.github/assets/icon.png`/`logo.png`自体が既にOpenMiQ本家の正規アセットのため。別ブランドとして自分の画像に差し替えたい場合のみ設定する（`ADDITIONAL_TERMS.md` §4の制約に従うこと） |
...
1. [`deploy/nginx/openmiq-api.conf`](../deploy/nginx/openmiq-api.conf)を配置する（自分のドメインでセルフホストする場合はファイル内の`miq.otnc.dev`をすべて置き換える）:
   ```bash
...
2. **General Information** → Interactions Endpoint URLに `https://miq.otnc.dev/api/discord/interactions` を設定して保存する。Discordはこの時点で実際にPINGインタラクションを送って検証するため、`apps/api`が起動済みで`DISCORD_PUBLIC_KEY`が正しく設定されている必要がある（保存が失敗する場合はpm2のログでエラーを確認）
3. **OAuth2** → Redirectsに `https://miq.otnc.dev/api/auth/discord/callback` を追加して保存する
...
- `https://miq.otnc.dev/api/about` — 帰属表示が返ることを確認
- `https://miq.otnc.dev/` — Web Consoleのトップページが表示されることを確認
- `https://miq.otnc.dev/api/docs` — Swagger UIが表示されることを確認
