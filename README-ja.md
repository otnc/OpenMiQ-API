...
6. 同じ`DISCORD_BOT_TOKEN`は実行時にも必須です — ヘッダーに表示するログイン中ユーザー自身のアバター、Admin画面のユーザー一覧に表示する各ユーザーのアバター、トップページのサンプルクォート画像（`ADMIN_DISCORD_IDS`の先頭ユーザーのアバター、起動時に一度だけ取得）のすべてが、Discordの`GET /users/{id}`（上記のWebhook/ギルド設定とは無関係な、グローバルなエンドポイント）経由でこのトークンを使います。
