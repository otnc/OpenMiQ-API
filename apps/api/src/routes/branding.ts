...
// No fallback to the bundled OpenMiQ icon/logo here on purpose: those assets identify the original project and its author specifically (ADDITIONAL_TERMS.md §4), and defaulting to them for every fork that doesn't set ICON_PATH/LOGO_PATH would put the original branding on deployments that aren't the original — including forks distributed under a different name (§2), which the terms don't allow at all. Every deployment, this project's own official one included, sets ICON_PATH/LOGO_PATH explicitly if it wants an icon/logo served.
