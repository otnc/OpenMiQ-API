...
describe("OpenMiQ constructor", () => {
  it("requires an apiKey", () => {
    expect(() => new OpenMiQ({ apiKey: "" })).toThrow(ValidationError);
    expect(() => new OpenMiQ({} as never)).toThrow(ValidationError);
  });
});
...
describe("OpenMiQ requests", () => {
  ...
  it("sends the API key as X-API-Key and hits /api/quote by default", async () => {
    ...
    const buffer = await new OpenMiQ({ apiKey: "sk_test_123" })
      .setText("hi")
      .setUsername("alice")
      .toBuffer();
    ...
    expect(url).toBe("https://miq.otnc.dev/api/quote");
  ...
  it("hits /api/fakequote when setFake() is used", async () => {
    ...
    expect(url).toBe("https://miq.otnc.dev/api/fakequote");
  });
  ...
  it("respects a custom baseUrl", async () => {
    ...
    await new OpenMiQ({ apiKey: "k", baseUrl: "http://localhost:9413" })
      .setText("hi")
      .setUsername("alice")
      .toBuffer();

    const [url] = fetchMock.mock.calls[0] as [string];
    expect(url).toBe("http://localhost:9413/api/quote");
  });

  it("refuses to send before text/authorName are both set", async () => {
    await expect(new OpenMiQ({ apiKey: "k" }).toBuffer()).rejects.toThrow(
      ValidationError,
    );
  });
  ...
  it("toURL() posts with options.hosted and returns the url from the response", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ url: "https://miq.otnc.dev/api/images/abc" }),
        {
          status: 201,
    ...
    const url = await new OpenMiQ({ apiKey: "k" })
      .setText("hi")
      .setUsername("alice")
      .toURL();

    expect(url).toBe("https://miq.otnc.dev/api/images/abc");
    const [, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    const body = JSON.parse(init.body as string);
  ...
  it("toBuffer({ hosted: true }) uploads then downloads the bytes back", async () => {
    ...
    const buffer = await new OpenMiQ({ apiKey: "k" })
      .setText("hi")
      .setUsername("alice")
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toBe(
      "https://miq.otnc.dev/api/images/abc",
    );
  });
  ...
  it("getUsage() parses the usage summary", async () => {
    ...
    const usage = await new OpenMiQ({ apiKey: "k" }).getUsage();
  ...
  it("wraps a non-2xx response in OpenMiQApiError with the status attached", async () => {
    ...
    const promise = new OpenMiQ({ apiKey: "k" })
      .setText("hi")
      .setUsername("alice")
      .toBuffer();
