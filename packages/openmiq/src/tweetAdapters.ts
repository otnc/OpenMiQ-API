...
import type { TweetLike } from "./types.ts";
 ...
 * })
 * new OpenMiQ({ apiKey }).setFromTweet(fromTwitterApiV2Tweet(tweet, includes))
 * ```
 */
 ...
 * new OpenMiQ({ apiKey }).setFromTweet(fromFxTwitterStatus(status))
 * ```
 */
